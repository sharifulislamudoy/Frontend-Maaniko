  ...
  return (
    <main className="min-h-screen bg-[#fff9fb] py-8 md:py-12">
      <div className="mx-auto max-w-7xl px-4 md:px-6 lg:px-8">
        <div className="flex flex-col gap-4 rounded-[28px] bg-gradient-to-br from-[#FC5689] to-[#f27da1] p-5 text-white shadow-[0_18px_45px_rgba(239,66,119,.18)] md:flex-row md:items-center md:justify-between md:p-8">
          <div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <section className="rounded-3xl bg-white p-5 shadow-[0_8px_28px_rgba(6,42,84,.05)] md:p-6">
            ...
            <div className="mt-5 flex gap-2">
              <div className="flex h-12 flex-1 items-center rounded-xl bg-slate-50 px-4 font-black tracking-wider text-[#062a54]">
                {data.referral.code}
              </div>
              <button
                type="button"
                onClick={() => void copyReferral()}
                className="grid size-12 place-items-center rounded-xl bg-[#062a54] text-white"
              >
                <Copy className="size-4" />
              </button>
            </div>
            <div className="mt-3 flex gap-4 text-xs font-bold text-slate-500">
              <span>{data.referral.completed} successful</span>
              <span>{data.referral.pending} pending</span>
            </div>
            <form
            ...
            >
              ...
              <div className="mt-2 flex gap-2">
                <input
                  ...
                  className="h-11 flex-1 rounded-xl border border-slate-200 px-3 text-sm uppercase outline-none focus:border-[#FC5689]"
                />
                <button
                  disabled={applying || !referralCode.trim()}
                  className="rounded-xl bg-[#FC5689] px-4 text-xs font-black text-white disabled:opacity-50"
                >
                  Apply
                </button>
              </div>
              ...
              {data.reminders.map((item) => (
                ...
                >
                  ...
                  <button
                    ...
                    className={`rounded-lg px-3 py-2 text-[11px] font-black ${item.status === "CANCELLED" ? "bg-slate-200 text-slate-600" : "bg-emerald-50 text-emerald-700"}`}
                  >
        ...
        <section className="mt-6 rounded-3xl bg-white p-5 shadow-[0_8px_28px_rgba(6,42,84,.05)] md:p-6">
