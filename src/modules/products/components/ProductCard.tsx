...
import type { ProductCardProps } from "@/modules/products/types/product";
    ...
    <article className="group/card relative isolate flex h-full min-w-0 flex-col overflow-hidden rounded-[20px] border border-[#dce3ec] bg-white shadow-[0_12px_34px_rgba(6,42,84,0.08)] transition duration-500 hover:-translate-y-1 hover:border-[#FC5689]/25 hover:shadow-[0_20px_48px_rgba(6,42,84,0.14)]">
      <div className="relative aspect-[20/21] shrink-0 overflow-hidden bg-[#fff4f6]">
        ...
        <div className="pointer-events-none absolute left-2 top-2 z-10 flex max-w-[calc(100%-56px)] flex-wrap items-start gap-1.5 sm:left-3 sm:top-3 sm:gap-2">
          ...
          {product.badge && (
            <span
              className={`rounded-full border border-white/70 bg-white/90 px-2 py-1 text-[8px] font-extrabold uppercase text-[#062a54] shadow-sm backdrop-blur-md sm:px-2.5 sm:text-[9px] ${"tracking-normal"}`}
            >
              {text(product.badge)}
            </span>
          )}
        ...
        <div className="buy-now-overlay absolute inset-x-0 bottom-0 z-20">
          ...
          >
            <span className="flex items-center gap-1.5 transition-all duration-300 group-hover/buy:-translate-y-8 group-hover/buy:opacity-0 sm:gap-2">
              {isAvailable && (
                <ShoppingCartIcon className="size-4 fill-current sm:size-5" />
              )}
              {isAvailable ? buyNowLabel : t("product.outOfStock")}
            </span>

            {isAvailable && (
              <ShoppingCartIcon className="pointer-events-none absolute size-6 translate-y-8 fill-current opacity-0 transition-all duration-300 group-hover/buy:translate-y-0 group-hover/buy:opacity-100" />
            )}
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col space-y-1 p-2.5 sm:p-4">
        <div className="flex min-w-0 items-center justify-between gap-1.5 sm:gap-3">
          <p
            className={`min-w-0 flex-1 truncate text-[8px] font-extrabold uppercase text-[#03A7FD] sm:text-[10px] ${"tracking-normal"}`}
          >
            {productCategory}
          </p>
