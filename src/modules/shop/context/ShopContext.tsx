...
export function ShopProvider({ children }: ShopProviderProps) {
  ...
  const addCustomComboToCart = useCallback(
    async (
      ...
      const key =
        product.id === "custom-solution-box"
          ? "COMBO:CUSTOM-SOLUTION-BOX"
          : `COMBO:${product.id}:CUSTOM`;
      const customizedProduct: MaanikoProduct = {
        ...product,
        price: unitPrice,
        comboItems: (product.comboItems ?? []).filter((item) =>
          config.some((candidate) => candidate.productId === item.productId),
        ),
      };
