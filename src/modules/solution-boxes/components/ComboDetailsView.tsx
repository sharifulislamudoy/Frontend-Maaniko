...
import ComboIncludedProducts from "@/modules/solution-boxes/components/ComboIncludedProducts";
...
type ComboDetailsTab =
  "overview" | "guide" | "selection" | "reviews" | "support";
...
export default function ComboDetailsView({
  ...
  const copy = {
    ...
    packagingDescription:
      "পরিষ্কার, protective এবং gift-ready presentation—যাতে box খোলার মুহূর্তটিও বিশেষ হয়।",
    reviewTitle: "এই Combo নিয়ে মায়েদের অভিজ্ঞতা",
    verified: "Verified purchase",
    deliveryTitle: "Delivery, Support ও Replacement",
    deliveryDescription:
      "ঢাকার ভিতরে ও বাইরে যত্নসহ delivery। কোনো item missing বা damaged হলে দ্রুত support team-কে জানান। Customer-এর অনুমতি ছাড়া কোনো item substitute করা হবে না।",
    ...
    itemCount: includedItems.reduce(
      (sum, item) => sum + item.relation.quantity,
      0,
    ),
  };
  ...
  return (
    <main className="min-h-screen bg-[#fff9fb] pb-36 pt-2 text-[#062a54] md:pb-40 md:pt-5 xl:pb-12">
      <div className="mx-auto w-full max-w-7xl space-y-3 overflow-x-hidden px-4 md:space-y-5 lg:px-8">
        ...
        <section className="grid min-w-0 items-start gap-3 md:gap-5 lg:grid-cols-[0.92fr_1.08fr] lg:items-stretch">
          ...
          <div className="flex h-full min-w-0 flex-col rounded-2xl border border-[#dce3ec] bg-white p-3 shadow-[0_12px_36px_rgba(6,42,84,0.06)] md:p-5 lg:p-6">
            ...
            <div className="mt-3 flex items-center gap-2 text-xs font-bold md:mt-4 md:text-sm">
              <CheckCircle2
                className={`size-4 ${isAvailable ? "text-emerald-500" : "text-slate-400"}`}
              />
              <span
                className={isAvailable ? "text-emerald-600" : "text-slate-400"}
              >
            ...
            <Link
              href={`/solution-box/customised?from=${encodeURIComponent(box.slug)}`}
              className="mt-3 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#03A7FD] bg-[#03A7FD]/10 px-4 text-sm font-black text-[#0278ad] transition hover:bg-[#03A7FD] hover:text-white"
            >
        ...
        <section aria-label={copy.detailsTabs}>
          ...
          >
            ...
            {activeTab === "guide" && (
              <GuideSection title={copy.guideTitle} steps={box.usageGuide} />
            )}
        ...
        {relatedBoxes.length > 0 && (
          <section
            ...
            aria-labelledby="related-combos-title"
          >
            <h2
              id="related-combos-title"
      ...
      <div className="fixed inset-x-0 bottom-[calc(64px+env(safe-area-inset-bottom))] z-40 border-t border-[#f0d9e1] bg-white/95 px-2 py-1.5 shadow-[0_-8px_24px_rgba(6,42,84,0.09)] backdrop-blur-xl md:bottom-[calc(76px+env(safe-area-inset-bottom))] md:px-3 xl:hidden">
        ...
        >
          <ShoppingCartIcon className="size-3.5 fill-current" />
          {isAvailable
            ? `${copy.orderNow} • ${priceFormatter.format(box.price)}`
            : copy.unavailable}
        </button>
      </div>
    </main>
    ...
    >
      ...
      <ul className="mt-4 grid gap-2.5 md:mt-5">
        {items.map((item, index) => (
          <li
            key={index}
            className="flex gap-2.5 rounded-xl border border-[#f1dfe5] bg-[#fff9fb] p-3 text-xs leading-5 text-slate-600 md:text-sm md:leading-6"
          >
...
function GuideSection({
  ...
  title: string;
  steps: SolutionBox["usageGuide"];
}) {
  const { text } = useSiteText();

  return (
    <section className="rounded-2xl border border-[#dce3ec] bg-[#fff9fb] p-4 md:p-6">
      <SectionTitle icon={BookOpenCheck} title={title} />
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-2 gap-2.5 md:mt-5">
        {items.map((item, index) => (
          <article
            key={index}
            className="flex min-h-24 flex-col items-center justify-center rounded-xl border border-[#dcebf2] bg-[#f4fbff] p-3 text-center text-[11px] font-bold leading-4 md:min-h-28 md:text-sm md:leading-5"
          >
