...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  function handleAddToCart() {
    ...
    if (resetTimerRef.current) clearTimeout(resetTimerRef.current);
    resetTimerRef.current = setTimeout(() => setCartStatus("idle"), 1500);
  }

  return (
    <article className="group/card relative isolate overflow-hidden rounded-[20px] border border-[#dce3ec] bg-white shadow-[0_12px_34px_rgba(6,42,84,0.08)] transition duration-500 hover:-translate-y-1 hover:border-[#ef4277]/25 hover:shadow-[0_20px_48px_rgba(6,42,84,0.14)]">
      <div className="relative aspect-[20/21] overflow-hidden bg-[#fff4f6]">
        <Link
          href={productHref}
        ...
        <button
          ...
          className={`absolute right-3 top-3 z-20 inline-flex size-10 items-center justify-center rounded-full border border-white/80 bg-white/90 shadow-[0_8px_22px_rgba(6,42,84,0.16)] backdrop-blur-md transition-all duration-300 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#ef4277]/20 xl:translate-y-2 xl:opacity-0 xl:group-hover/card:translate-y-0 xl:group-hover/card:opacity-100 ${
            wished ? "text-[#ef4277]" : "text-[#062a54] hover:text-[#ef4277]"
          }`}
        >
          <Heart
        ...
        <div className="absolute bottom-0 z-20 w-full translate-y-0 opacity-100 transition-all duration-500 ease-out xl:translate-y-[calc(100%+20px)] xl:opacity-0 xl:group-hover/card:translate-y-0 xl:group-hover/card:opacity-100">
          <button
            ...
            className={`group/cart relative flex h-11 w-full items-center justify-center overflow-hidden rounded-t-2xl px-5 text-sm font-extrabold shadow-[0_12px_30px_rgba(6,42,84,0.22)] transition-all duration-300 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#10a9e8]/25 ${
              isAvailable
      ...
      <div className="space-y-2.5 p-4">
        ...
        <div>
          <Link href={productHref}>
            <h2 className="line-clamp-2 min-h-11 text-base font-bold leading-[22px] text-[#062a54] transition-colors duration-300 hover:text-[#ef4277]">
              {productName}
            </h2>
          </Link>
        ...
        <div className="flex items-end justify-between gap-3 border-t border-[#dce3ec]/80 pt-2.5">
