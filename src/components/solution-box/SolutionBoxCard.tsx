...
export default function SolutionBoxCard({ box }: SolutionBoxCardProps) {
  ...
  const [primaryImage, secondaryImage] = box.images;
  ...
  return (
    <article className="group/card relative isolate flex h-full flex-col overflow-hidden rounded-[20px] border border-[#dce3ec] bg-white shadow-[0_12px_34px_rgba(6,42,84,0.08)] transition duration-500 hover:-translate-y-1 hover:border-[#ef4277]/25 hover:shadow-[0_20px_48px_rgba(6,42,84,0.14)]">
      <div className="relative aspect-[20/21] shrink-0 overflow-hidden bg-[#fff4f6]">
        ...
        <div className="absolute inset-x-0 bottom-0 z-20 translate-y-full opacity-0 transition-all duration-300 ease-out group-hover/card:translate-y-0 group-hover/card:opacity-100 group-focus-within/card:translate-y-0 group-focus-within/card:opacity-100">
          <Link
            href={box.href}
            aria-label={accessibilityLabel}
      ...
      <div className="flex flex-1 flex-col p-3 sm:p-4">
        <Link href={box.href} className="group/title">
        ...
        <p className="mt-auto border-t border-[#dce3ec]/80 pt-2.5 text-base font-black text-[#ef4277] sm:text-lg">
